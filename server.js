...
app.get('/health', (_req, res) => res.json({ ok: true, rooms: rooms.size, now: Date.now(), netcode: 'server-authoritative-v8' }));
...
function roomState(room) {
  ...
  return {
    ...
    netcode: 'server-authoritative-v8'
  };
}
...
io.on('connection', socket => {
  socket.emit('server_hello', { id: socket.id, modes: MODES, defaults: DEFAULT_SETTINGS, netcode: 'server-authoritative-v8' });
  ...
  socket.on('player_input', ({ input, seq, clientTime } = {}) => {
    ...
    // Optional: echo newest input to other clients only for prediction/animation.
    socket.to(room.code).volatile.emit('player_input', { playerId: socket.data.playerToken, input: safe, seq: Number(seq) || 0, clientTime: Number(clientTime) || 0 });
  });

  // V8: Clients no longer authoritatively send snapshots. Kept for backwards compatibility, ignored.
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Rocket League 2D Pro Online V8 läuft auf Port ${PORT} (server-authoritative netcode)`);
});
